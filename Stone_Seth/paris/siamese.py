...
def embedding_model(
    ...
    attention_layer : Layer, optional
        attention layer added to first Conv2D layer
    attention_idx : int, optional
        index of layer to which attention layer will be concatenated with
        pass outshape layer of attention layer bearning this in mind
        index only applies to convolutional layers i.e. model_layers[:-1]
    ...
    if custom_layers is None:
        ...
        # take the same input to the attention layer
        if attention_layer is not None:
            # need to flatten output before feeding
            attn_in = keras.layers.Flatten()(conn)
            # pass input to anntention layer
            # save output to new variable
            attn_out = attention_layer(attn_in)

            attn_out = keras.layers.Conv2D(model_layers[0], 
                                           kernel_size)(attn_out)
            attn_out = keras.layers.Dropout(0.1)(attn_out)

        # add conv body layers
        for i, num_filter in enumerate(model_layers[:-1]):

            # concatenate attention here
            if i == attention_idx and attention_layer is not None:
                conn = tf.concat([conn, attn_out], -1)
...
if __name__ == "__main__":
    ...
    exit()
    print("Running tests defined in siamese.py")
    print("CUSTOM MODEL TEST for embedding_model()")

    x = np.random.random((500, 10, 10, 1))
    y = np.random.randint(0, 19, size=(500, 1))

    from attention import FingerprintAttention

    kx = x.reshape((-1, 100))
    vx = y.flatten()

    fa = FingerprintAttention(kx, vx, outshape=(9, 9))

    model = triplet_encoder = embedding_model(
        (10, 10, 1),
        20,
        attention_layer=fa,
        attention_idx=1,
